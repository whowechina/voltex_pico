...
* **以下仅适用于霍尔效应（磁轴）版本。**
  ...
  * 1x LM4040 并联电压基准，请选用 2.0V、2.1V 或 2.5V 基准电压的型号，SOT23-3 封装 (U6)。
...
### 组装
#### 需要的其他组件
* 6x **M2.5\*(14mm-16mm) 螺丝** 用于将磁铁座固定到旋钮。
* 8x **M2\*6mm 螺丝** 用于固定轴承（轴承固定器到侧翼盖）。
* 6x **M2\*8mm 螺丝** 用于将侧翼底座固定到侧翼盖，如果使用了热熔螺母则使用 **M2\*6mm**。
* 12x **M3\*(4mm-5mm) 螺丝** 用于将 PCB 固定到底座部件。
* 10x **M3\*8mm 螺丝** 用于固定底座、侧翼底座、支撑和面板。如果使用了热熔螺母则使用 **M3\*6mm**。
* 2x 6702zz (15x21x4mm) 轴承用于旋钮。
...
* 按住左辅助按钮插入以进入 Konami 模拟模式；右侧用于常规模式。
